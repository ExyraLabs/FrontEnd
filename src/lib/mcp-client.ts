...
export class MCPClient implements MCPClientInterface {
  ...
  public async callTool(
    ...
    try {
      console.log(
        `[MCP Client] Calling tool: ${name} with args:`,
      ...
      console.log(
        `[MCP Client] Processed args for ${name}:`,
      ...
      console.log(
        `[MCP Client] Tool ${name} completed with result:`,
        JSON.stringify(result, null, 2)
      );

      return result;
    } catch (error) {
      console.error(`[MCP Client] Error calling tool ${name}:`, error);
